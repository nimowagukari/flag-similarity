...
- [ ] 動作確認（`infra/README.md`の手順でbootstrap → Terraform apply → Function URLへのcurl疎通 → CloudWatch Logsで応答時間確認 → Reserved Concurrencyによるスロットリング確認 → push後の反映時間確認）

### 次のアクション

1. `infra/README.md`の手順でstate用S3バケットを作成し、`terraform init`〜初回applyを実行
2. `.github/workflows/deploy.yml`を作成し、CI/CDの自動反映経路を繋ぐ
3. Function URL疎通・CloudWatch Dashboard・Budget通知の動作確認
