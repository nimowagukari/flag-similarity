...
flag-similarity を AWS Lambda（コンテナイメージ + [Lambda Web Adapter](https://github.com/aws/aws-lambda-web-adapter)）+ Lambda Function URL で動かすためのTerraform一式。
...
| `variables.tf` | `aws_region` / `project_name` / `github_repository` / `github_oidc_provider_arn` のみ変数化。それ以外の設定値（メモリサイズ、Reserved Concurrencyなど）は各`.tf`にハードコード（変更したい場合はコードを直接編集する） |
| `provider.tf` | AWSプロバイダ、default_tags |
| `ecr.tf` | アプリのコンテナイメージ用ECRリポジトリ・ライフサイクルポリシー |
| `lambda.tf` | Lambda関数(container image, arm64)・Function URL(公開)・ロググループ |
| `iam.tf` | Lambda実行ロール、GitHub Actionsデプロイロール（OIDCプロバイダ自体はこのTerraformの管理対象外。下記「前提条件」参照） |
| `outputs.tf` | Function URLなど、apply後に必要な値 |
...
```sh
terraform output function_url                 # アプリの公開URL
